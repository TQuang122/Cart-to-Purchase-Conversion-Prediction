"""
End-to-end Airflow DAG for cart-to-purchase pipeline - CatBoost.
Manual trigger only. CatBoost training + evaluation + registry promotion.
...
MODEL_CONFIG = f"{MODEL_PIPELINE_DIR}/src/config/catboost.yaml"
...
with DAG(
    dag_id="cart_to_purchase_e2e_catboost",
    description="End-to-end cart-to-purchase pipeline - CatBoost (manual trigger)",
    ...
    tags=["cart-to-purchase", "mlops", "catboost"],
) as dag:
    ...
    train_model = BashOperator(
        ...
        bash_command=(
            ...
            f"--run-name catboost_airflow_{{{{ run_id | replace(':', '_') | replace('+', '_') | replace(' ', '_') }}}} "
            '2>&1 | tee "$LOG_TMP" && '

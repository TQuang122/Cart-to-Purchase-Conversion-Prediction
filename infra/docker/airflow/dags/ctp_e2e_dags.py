...
HPO_BEST_PARAMS = f"{MODEL_PIPELINE_DIR}/logs/best_params.json"

# Number of Optuna HPO trials (override with Airflow Variable 'hpo_n_trials' if needed)
HPO_N_TRIALS = 150  # Increased from 50 for better hyperparameter search
...
with DAG(
    ...
    hpo_xgboost = BashOperator(
        task_id="hpo_xgboost",
        bash_command=(
            "set -euo pipefail && "
            f"mkdir -p {MODEL_PIPELINE_DIR}/logs && "
            f"python {MODEL_SCRIPTS_DIR}/hpo.py "
            f"--config {MODEL_CONFIG} "
            f"--training-data-path {TRAIN_DATA} "
            f"--n-trials {HPO_N_TRIALS} "
            f"--output-params-path {HPO_BEST_PARAMS}"
        ),
        env=DEFAULT_ENV,
    )
    ...
    train_model = BashOperator(
        ...
        bash_command=(
            ...
            f"--config {MODEL_CONFIG} "
            f"--training-data-path {TRAIN_DATA} "
            f"--best-params-path {HPO_BEST_PARAMS} "
            f"--run-name xgboost_airflow_{{{{ run_id | replace(':', '_') | replace('+', '_') | replace(' ', '_') }}}} "
            '2>&1 | tee "$LOG_TMP" && '
    ...
    split_train_test >> hpo_xgboost >> train_model >> extract_run_id

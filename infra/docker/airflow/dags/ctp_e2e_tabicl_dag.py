"""
End-to-end Airflow DAG for TabICL model training.
Manual trigger only. TabICL training + evaluation + registry promotion.
...
MODEL_CONFIG = f"{MODEL_PIPELINE_DIR}/src/config/tabicl_airflow.yaml"
...
MODEL_NAME = "tabicl_ctp"
ALIAS_NAME = "staging"

# TabICL recommended sample size for training
# Using 50K samples for balance between performance and memory
TABICL_SAMPLE_SIZE = 20000
...
with DAG(
    dag_id="cart_to_purchase_tabicl_e2e",
    description="End-to-end cart-to-purchase pipeline with TabICL (manual trigger)",
    ...
    tags=["cart-to-purchase", "mlops", "tabicl"],
) as dag:
    ...
    # Train TabICL model
    # Note: TabICL doesn't need HPO, so we train directly
    train_model = BashOperator(
        task_id="train_tabicl",
        bash_command=(
            "set -euo pipefail && "
            ...
            f"--training-data-path {FULL_DATA} "
            f"--sample-size {TABICL_SAMPLE_SIZE} "
            f"--run-name tabicl_airflow_{{{{ run_id | replace(':', '_') | replace('+', '_') | replace(' ', '_') }}}} "
            '2>&1 | tee "$LOG_TMP" && '
    ...
    # Extract run ID from training log

...
```bash
docker network create ctp-network
```
...
- **Node Exporter** (Port 9100) - Host metrics *(Linux only)*
- **DCGM Exporter** (Port 9400) - NVIDIA GPU metrics *(NVIDIA GPU only)*
...
**Platform-specific Services (using Docker Compose Profiles):**

Some monitoring services require specific hardware/OS:

| Service | Profile | Requirement |
|---------|---------|-------------|
| node_exporter | `linux` | Linux host (root filesystem mount) |
| dcgm-exporter | `nvidia` | NVIDIA GPU with nvidia-docker runtime |
...
# macOS (default - skips node_exporter and dcgm-exporter)
docker compose up -d
...
docker compose --profile linux --profile nvidia up -d
```
...
1. **Check Docker network exists:**
   ```bash
   docker network ls | grep ctp-network
   ```
...
1. **Verify MinIO bucket exists:**
   ```bash
   docker exec -it ctp_minio_mc mc ls minio/
   ```

2. **Check MySQL connection:**
   ```bash
   docker exec -it ctp_mysql mysql -u mlflow -p
   ```
...
1. **macOS: "unknown or invalid runtime name: nvidia" error:**
   - This occurs because `dcgm-exporter` and `node_exporter` are not compatible with macOS
   - These services use Docker Compose profiles and are automatically skipped on macOS
   - Just run: `docker compose up -d` (without profile flags)

2. **GPU metrics not showing:**
   - Verify NVIDIA Docker runtime is installed
   - Run with nvidia profile: `docker compose --profile nvidia up -d`

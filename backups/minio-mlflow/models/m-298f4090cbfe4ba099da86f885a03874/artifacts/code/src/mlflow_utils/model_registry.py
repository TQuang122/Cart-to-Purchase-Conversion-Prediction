    ...
    def retrieve_eval_metrics_based_on_run_id(
        ...
        try:
            ...
            if latest_eval.empty:
                logger.warning(f"No eval run found for run_id={run_id}")
                return None
            eval_run_id = latest_eval['run_id'].values.tolist()[0]
            eval_run = self.client.get_run(eval_run_id)
            return eval_run.data.metrics.get(metric, None)
        except Exception as e:
            logger.warning(f"Could not retrieve eval metric '{metric}' for run_id={run_id}: {e}")
            return None

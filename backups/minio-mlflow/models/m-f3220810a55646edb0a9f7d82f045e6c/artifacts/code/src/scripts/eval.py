...
def save_predictions_with_probabilities(
    ...
    logger.info(
        f"  - Predicted positives: {output_df['prediction'].sum()} ({output_df['prediction'].mean():.2%})"
    )
...
def main():
    ...
    logger.info(
        f"Loaded {len(eval_data)} samples with {len(eval_data.columns)} features"
    )
    ...
    with tracker.start_run(
        ...
        if args.validate_thresholds:
            logger.info(
                f"Threshold Validation: {'PASSED' if validation_passed else 'FAILED'}"
            )  # type:ignore
